...
- `-v $HOME/cicada/data:/data` 表示宿主的 `$HOME/cicada/data` 目录映射容器的 `/data` 目录到, 知了容器使用 `/data` 保存数据
- `-v $HOME/cicada/config.json:/config/cicada.json:ro` 表示宿主的 `$HOME/cicada/config.json` 文件映射容器的 `/config/cicada.json` 文件, 知了容器配置文件位于 `/config/cicada.json`, `ro` 表示只读. 如果你的环境不支持文件映射, 以这个例子为例可以使用目录映射 `-v $HOME/cicada:/config:ro`
