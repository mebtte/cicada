...
// upload IDs are generated by the server as 32 lower-case hex characters.
var partialUploadIDRegexp = regexp.MustCompile(`^[0-9a-f]{32}$`)
...
func InitPartialUpload(c *gin.Context) {
	...
	api.OK(c, gin.H{
		...
		"expiresAt":     partialUploadExpiresAt(stored),
	})
}
...
func GetPartialUpload(c *gin.Context) {
	...
	uploadID, ok := partialUploadIDParam(c)
	if !ok {
		return
	}
	...
	api.OK(c, gin.H{
		...
		"expiresAt":     partialUploadExpiresAt(meta),
	})
}
...
func PutPartialUploadChunk(c *gin.Context) {
	...
	if start != meta.ReceivedBytes {
		api.OK(c, partialUploadProgressPayload(meta))
		return
	}
	...
	if err != nil {
		...
		case errors.Is(err, musicasset.ErrRangeOutOfOrder):
			if current, readErr := musicasset.ReadSession(uploadID); readErr == nil && current.UserID == user.ID {
				api.OK(c, partialUploadProgressPayload(current))
				return
			}
			api.Fail(c, apperr.PartialUploadRangeInvalid)
		case errors.Is(err, musicasset.ErrRangeOverflow):
			api.Fail(c, apperr.PartialUploadRangeInvalid)
		default:
			api.Fail(c, apperr.ServerError)
		}
...
func partialUploadIDParam(c *gin.Context) (string, bool) {
	uploadID := c.Param("uploadId")
	if !partialUploadIDRegexp.MatchString(uploadID) {
		api.Fail(c, apperr.WrongParameter)
		return "", false
	}
...
func partialUploadExpiresAt(meta musicasset.PartialUploadMeta) int64 {
	return meta.UpdatedAt + int64(musicasset.PartialUploadTTL/time.Millisecond)
}

func partialUploadProgressPayload(meta musicasset.PartialUploadMeta) gin.H {
	return gin.H{
		"uploadId":      meta.UploadID,
		"size":          meta.Size,
		"receivedBytes": meta.ReceivedBytes,
		"nextOffset":    meta.ReceivedBytes,
	}
}
