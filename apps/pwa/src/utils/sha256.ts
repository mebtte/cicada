...
export function sha256Hex(input: Uint8Array): string {
  ...
  // SHA-256 stores the original message length as a big-endian 64-bit value.
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    // Add the compressed chunk to the running hash state modulo 2^32.
    hash[0] = (hash[0] + a) >>> 0;
    hash[1] = (hash[1] + b) >>> 0;
    hash[2] = (hash[2] + c) >>> 0;
    hash[3] = (hash[3] + d) >>> 0;
  ...
  return Array.from(hash).map(hex32).join('');
}
