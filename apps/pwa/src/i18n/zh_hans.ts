...
const zhCN: {
  ...
  offline_cache_help:
    '离线缓存分两层:\n\n• 音乐文件: 任何播放过的歌曲都会自动缓存到本地, 离线时优先从本地播放, 不消耗流量\n• 音乐数据: 主动点"下载到本地", 或者完整播放完一首歌后, 该歌会进入此列表, 离线时可在此找到并播放\n\n注意事项:\n\n• 缓存按音质区分, 切换流畅度后同一首歌在新音质下需要重新缓存; 旧音质字节仍会保留, 浏览器在空间紧张时自动回收\n• 容量上限由浏览器根据设备空间动态分配, 不是固定值; 浏览器空间紧张时会优先清理较少使用的缓存\n• 把应用安装到桌面/主屏后, 缓存抗清理能力会显著提升',
  music_type_short: '类型',
  music_type_song: '歌曲',
